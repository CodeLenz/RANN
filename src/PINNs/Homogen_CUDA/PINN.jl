...
#
# -----------------------------------------------------------------------------
function Perda_Energia_Alvo(AL::AbstractMatrix{T}, pontos::AbstractMatrix{T}, ε_macro::AbstractMatrix{T}, prob::String,
    ...
    #
    # O material NÃO depende de AL. Envolvemos o bloco em Zygote.@ignore para
    # que ele seja tratado como constante no pullback (e para que a cópia cu
    # para a GPU não entre na diferenciação automática).
    #
    C11, C22, C12, C33 = Zygote.@ignore begin
        props_simbolo = Symbol("Propriedades_Material_"*prob)
        props = getfield(Main, props_simbolo).(pontos[:, 1], pontos[:, 2], Ref(mat_params))

        # Separa os vetores de E e ν (em lote) e envia para o dispositivo ativo
        # para casar com os deslocamentos que vêm da rede
        E_C = to_device(first.(props))
        ν_C = to_device(last.(props))

        # Construção termo a termo de C, com vetorização
        fac = E_C ./ (T(1.0) .- ν_C.^2)
        (fac, fac, fac .* ν_C, fac .* (T(1.0) .- ν_C) ./ T(2.0))
    end
