...
function Treina_Rede_PINN_Energia!(rede::Rede{T}, base_pontos::Matrix{T}, ε_macro::Matrix{T},
    ...
    # Aloca matriz de pontos com amostragem por round
    pontos = similar(base_pontos)
    ...
    for r = 1:rounds
        ...
        # Pontos novos desta rodada (QMC rotacionado)
        shift = T.(Sobol.next!(seq))
        pontos .= Gera_Pontos_QMC_Rotacionado(base_pontos, shift)
        ...
        for p in 1:N_pts
            ...
            for (j,pt) in enumerate(coords)
                X_all[:, 5*(p-1) + j] .= Camada_Periodica(pt[1], pt[2], N_modos)
            end
